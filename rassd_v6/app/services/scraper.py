...
def _parse_date(s):
    """Essaie de parser une date dans différents formats. Retourne None si aucun ne marche."""
    s = str(s).strip().split()[0]
    for fmt in DATE_FMT:
        try:
            return datetime.strptime(s, fmt).date()
        except ValueError:
            # Format ne correspond pas, on essaie le suivant
            continue
    return None
...
def _find_max_binary(s, low, high, log_fn) -> int:
    ...
    while low <= high:
        ...
        try:
            ...
            # Timeout ou erreur réseau: on considère cet ID comme invalide
            logger.debug(f"[find_max_id] ID {mid} erreur réseau: {e}")
            high = mid - 1
    ...
    for kid in known_ids:
        if kid.startswith("bdc_"):
            try:
                ...
                # ID mal formé (ex: "bdc_abc"), on l'ignore
                continue
