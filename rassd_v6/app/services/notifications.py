...
def build_tg_message(t: dict) -> str:
    ...
    lines += [
        ...
        f"📱 <a href='{cfg.SITE_URL}/tenders/{t['id']}'>Voir sur SOURCE</a>",
        "",
        "<i>SOURCE · Veille Marchés Publics Maroc</i>",
    ]
    return "\n".join(lines)
...
<body><div class="wrap">
<div class="hdr"><div class="logo">SOURCE</div><div style="font-size:10px;color:#555;letter-spacing:2px;font-family:monospace;margin-top:3px">MARCHÉS PUBLICS MAROC</div></div>
<div class="body">
...
<a href="{site}/tenders/{t["id"]}" class="cta2">Détails SOURCE</a>
</div>
<div class="ftr"><p style="color:#444;font-size:11px">SOURCE · <a href="{site}" style="color:#666">atlas.ma</a> · <a href="{site}/settings" style="color:#666">Gérer mes alertes</a></p></div>
</div></body></html>"""
...
def test_notifications(email: str = "", telegram_id: str = "") -> dict:
    ...
    fake_tender = {
        "id":               "bdc_test",
        "objet":            "TEST — Marché de test SOURCE",
    ...
    if email:
        ...
        results["email"] = email_send(email, "🧪 Test SOURCE — Notifications", html)
    return results
